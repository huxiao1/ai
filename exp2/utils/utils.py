...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    # Define criteria
    BCEcls = nn.BCEWithLogitsLoss(pos_weight=ft([h['cls_pw']])) #positive weight hyperparameter
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        ...
        if nb:  # number of targets
            ps = pi[b, a, gj, gi]  # prediction subset corresponding to targets: 85 elements for p,x,y,h,w,80 cls
