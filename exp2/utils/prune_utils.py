...
def parse_module_defs2(module_defs):
    ...
    Conv_idx = []
    shortcut_idx=dict()
    shortcut_all=set() #set() 函数创建一个无序不重复元素集，可进行关系测试，删除重复数据，还可以计算交集、差集、并集等。
    ignore_idx = set()
    for i, module_def in enumerate(module_defs): #iterate each layer
        ...
        if module_def['type'] == 'convolutional':
            ...
            if module_defs[identity_idx]['type'] == 'convolutional':
                ...
                #ignore_idx.add(identity_idx)
                shortcut_idx[i-1]=identity_idx #why there is a -1?
                shortcut_all.add(identity_idx)
            elif module_defs[identity_idx]['type'] == 'shortcut':
                
                #ignore_idx.add(identity_idx - 1)
                shortcut_idx[i-1]=identity_idx-1 #shortcut idx is marked as conv layer idx?
...
def gather_bn_weights(module_list, prune_idx):
    ...
    #print([module_list[idx][1].bias.data.shape for idx in prune_idx])
...
def obtain_bn_mask(bn_module, thre):
    ...
    thre = thre.cuda()
    mask = bn_module.weight.data.abs().ge(thre).float() #element wise comparison 
